...
(new SingleCommandApplication())
    ->setCode(function (InputInterface $input, OutputInterface $output) {
        ...
        $client->request('GET', $url)->then(
            function (Psr\Http\Message\ResponseInterface $response) use ($outputDir, $tempDir, $logger, $url) {
            ...
            $bodyStream->on('data', function ($chunk) use ($filePointer, $logger, $fileName) {
                fwrite($filePointer, $chunk);
                $logger->info("Downloading {$fileName}...");
            });

            $bodyStream->on('end', function () use ($filePointer, $tempPath, $outputPath, $logger, $fileName) {
                fclose($filePointer);
                rename($tempPath, $outputPath);
                $logger->info("Downloaded file '{$fileName}' successfully.");
            });

            $bodyStream->on('error', function (Exception $e) use ($filePointer, $logger, $fileName) {
                fclose($filePointer);
                $logger->critical("Failed to download file '{$fileName}': " . $e->getMessage());
            });
            },
            function (Exception $e) use ($logger) {
            $logger->critical('Failed to download file: ' . $e->getMessage());
            }
        );
